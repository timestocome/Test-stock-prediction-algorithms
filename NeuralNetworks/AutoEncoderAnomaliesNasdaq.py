...
data = data[['LogOpen', 'dx', 'LogVolume']]
data = data.dropna()
...
data['LogVolume'] = data['LogVolume'] - data['LogVolume'].min()
...
errors = np.asarray(errors) * 3 + 1.5

plt.figure(figsize=(16,16))
plt.plot(errors, label='Deviation from expected', alpha=0.4, lw=2)
plt.plot(index, label='Nasdaq')
plt.legend(loc='best')
plt.title("Look for anomalies in Nasdaq using an auto encoder")
plt.grid('on')

plt.savefig("nasdaq_autoencoder.png")
plt.show()
