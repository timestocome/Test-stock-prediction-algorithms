...
def train(stock):
    ...
    # try one day price prediction 
    day_model = LinearRegression()
    train, test, target, test_target = train_test_split(stock[features], stock['next_day'], test_size=0.2)
    day_model = day_model.fit(train, target)
    predictions = day_model.predict(test)
    score = day_model.score(test, test_target)
    print('Accuracy on next day price:', score)
    ...
    return day_model, week_model, month_model, quarter_model
...
dj_day, dj_week, dj_month, dj_quarter = train(dj_train)

print("S&P")
sp_stock, sp_train, sp_test = read_data('data/vma_S&P.csv')
sp_day, sp_week, sp_month, sp_quarter = train(sp_train)

print("Russell")
r_stock, r_train, r_test = read_data('data/vma_Russell2000.csv')
r_day, r_week, r_month, r_quarter = train(r_train)

print("NASDAQ")
n_stock, n_train, n_test = read_data('data/vma_nasdaq.csv')
n_day, n_week, n_month, n_quarter = train(n_train)
...
print("DJIA Actual Jan 9th, 2017: ", dj_actual['Close'])
...
predict_w = n_week.predict((dj_stock.ix[row + one_week])[features])
predict_m = n_month.predict((dj_stock.ix[row + one_month])[features])
predict_q = n_quarter.predict((dj_stock.ix[row + one_quarter])[features])
